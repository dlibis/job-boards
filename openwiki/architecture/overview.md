...
Descriptions are intentionally bounded for the CLI. Ashby and Lever return description text in the normal board payload; Greenhouse requires `?content=true`, which the README documents as roughly 26x the bytes for a measured board, and Comeet requires `details=true`, measured at roughly 3.8x the bytes on a real board — for Comeet, `job["details"]` is a list of named sections (`Description`, `Responsibilities`, ...) that `normalize_comeet()` joins by `value`, in `order`. `scan_board()` only requests Greenhouse content or Comeet details when `--grep` is set, strips markup through `plain_text()`, stores at most two surrounding fragments from `fragments()`, and never includes whole descriptions in the CLI's output rows. The in-process collector seam is unbounded by design: `dispatch_board()` always passes `want_content=True`, for every provider, regardless of `--grep`, so the collector pays for and persists a description for every job a source can supply one for — a real, recurring bandwidth cost accepted deliberately, distinct from the CLI's opt-in default.
